...
func _choice_button_pressed(target_node: Node, wait_for_signal_to_continue: String):
	...
	# You can add a button press AudioStreamPlayer to the dialogue system,
	# then play that AudioStreamPlayer here! (15:03)
...
func _text_resource(i: DialogueText) -> void:
	...
	DialogueLabel.text = i.text
	var text_without_sqaure_brackets: String = _text_without_square_brackets(i.text)
	var total_characters: int = text_without_sqaure_brackets.length()
	...
	while DialogueLabel.visible_characters < total_characters:
		...
		if character_timer >= (1.0 / i.text_speed) or text_without_sqaure_brackets[DialogueLabel.visible_characters] == " ":
			var character: String = text_without_sqaure_brackets[DialogueLabel.visible_characters]
			DialogueLabel.visible_characters += 1
			if character != " ":
